...
namespace RakNet.Network
{
    public abstract class Write : Stream
    {
		...
		public void String(string val)
		{
			...
			buffer.Position = 0L;
			buffer.SetLength((long)buffer.Capacity);
			int bytes = Encoding.UTF8.GetBytes(val, 0, val.Length, buffer.GetBuffer(), 0);
			buffer.SetLength((long)bytes);
			this.BytesWithSize(buffer);
		}
		...
		public void BytesWithSize(MemoryStream val)
		{
			...
			this.UInt32((uint)val.Length);
			this.Write(val.GetBuffer(), 0, (int)val.Length);
		}

		// Token: 0x06000075 RID: 117 RVA: 0x00002BD0 File Offset: 0x00000DD0
		public void BytesWithSize(byte[] b)
		{
			using (MemoryStream memoryStream = new MemoryStream(b, 0, b.Length, true, true))
			{
				memoryStream.SetLength((long)b.Length);
				this.BytesWithSize(memoryStream);
			}
		}
