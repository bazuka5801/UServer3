...
namespace RakNet
{
    internal class StreamWrite : Write
    {
		...
		public override void Send(SendInfo info)
		{
			if (info.connections != null)
			{
				for (int i = 0; i < info.connections.Count; i++)
				{
					...
					if (memoryStream.Length > 1L && this.net.cryptography != null && this.net.cryptography.IsEnabledOutgoing(connection))
					{
			...
			if (info.connection != null)
			{
				...
				MemoryStream memoryStream2 = this.stream;
				if (memoryStream2.Length > 1L && this.net.cryptography != null && this.net.cryptography.IsEnabledOutgoing(connection2))
				{
					this.net.cryptography.Encrypt(connection2, memoryStream2, 1);
				}
				this.peer.SendStart();
				this.peer.WriteBytes(memoryStream2);
				this.peer.SendTo(connection2.guid, info.priority, info.method, info.channel);
			}
		}
		...
		{
			this.stream.Write(val, 0, val.Length);
		}
